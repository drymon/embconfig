        ...
        ctext = '''
{
	...
	if (file == NULL) {
		UB_LOG(UBL_WARN, "%s:Unable to open file \%s for reading.\\n", __func__, file_name);
		return -1;
	}
        ...
        ctext = '''
{
	...
	if (file == NULL) {
		UB_LOG(UBL_ERROR, "%s:Unable to open file\\n", __func__);
		return -1;
	}
'''
        outf.write(ctext)
...
'''
	...
	for (i = 0; i < nbuffs; i++) {
		...
		if (strstr(buffs[i], tag) != NULL) {
			...
			if (value == NULL) {
				UB_LOG(UBL_ERROR, "%s:Missing value: %s\\n", __func__, buffs[i]);
				return -1;
			}
			value += 1; //skip '='
			endtag = strstr(value, ";");
			if (endtag == NULL) {
				UB_LOG(UBL_ERROR, "%s:Missing ';': %s\\n", __func__, buffs[i]);
				return -1;
			}
			*endtag = 0;
'''
...
'''
        ...
        for data_type in data_types:
            outf.write("static int read_%s(%s *var, const char *tag, "
					   ...
					   FILE *file, char *bufread, int bufsize)
{
	...
	if (name_size >= REGION_NAME_SIZE) {
		UB_LOG(UBL_ERROR, "%s:name region too long: %s\\n", __func__, name);
		return -1;
	}
	...
	while (conf_readline(file, bufread, bufsize) > 0) {
		...
		if (region->buffs[region->nbuffs] == NULL) {
			UB_LOG(UBL_ERROR, "%s:unable to malloc\\n", __func__);
			break;
		}
	...
	if (end_region == false) {
		...
		UB_LOG(UBL_ERROR, "%s:Missing '};' region: %s\\n", __func__, region->name);
	...
	while (conf_readline(file, buff, sizeof(buff)) > 0) {
		...
		*temp = 0; //Set NULL terminate 
		...
		if (load_region(&regions[nregions], buff, temp - buff,
			...
			UB_LOG(UBL_ERROR, "%s:Unable to load region: %s\\n", __func__, region_name);
		}
...
'''
    ...
    def generate_include_header(self, outf):
        ...
        outf.write("#include <xl4unibase/unibase.h>\n")
...
if __name__ == "__main__":
    ...
    hfile_out.write("//This file is generated automatically. Do not edit manually!\n\n")
    ...
    cfile_out.write("//This file is generated automatically. Do not edit manually!\n\n")
